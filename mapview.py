...
def build_map(rd: Optional[dict], *, leaflet_prefix: str,
    ...
    white zoom control. fit_bounds alone can't fully close the margin --
    the AOI is a tall, narrow rectangle in Mercator at this latitude
    (~1.7:1 portrait) inside a landscape panel, an inherent geometry
    mismatch -- so the leftover space is themed dark instead of left as
    Leaflet's default void, which is what actually read as broken.
    """
